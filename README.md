...
PULSE 每日训练是一个部署在 Cloudflare Pages 上的私人运动跟练网站。它适合把 OpenList 里的训练视频或其它可播放的视频直链整理成训练库，支持播放、续看、系列管理、训练记录和跨设备进度同步。
...
- OpenList 集成：目录浏览、批量导入、目录访问密码、全局 token
- 普通视频直链：支持非 OpenList 的 http(s) 视频链接直接播放
...
- 加密：OpenList 目录密码使用 `SESSION_SECRET` 派生 AES-GCM 密钥加密存储
...
ALIST_TOKEN=optional-openlist-token
```
...
- `ALIST_BASE`：OpenList 地址，批量浏览、导入相对路径、刷新 OpenList 签名时需要。变量名沿用 `ALIST_BASE` 是为了兼容旧配置
- `ALIST_TOKEN`：可选，用于访问需要登录账号的 OpenList 私有资源。变量名同样保留旧命名
...
`ALIST_TOKEN` 可选；公开资源、普通视频直链或只使用目录密码时可以不配置。

## 使用流程

1. 打开网站并输入访问密码
2. 进入管理页添加单个视频直链，或使用批量添加浏览 OpenList 目录
...
1. OpenList 资源：配置 `ALIST_BASE` 后，可以浏览目录、批量导入，并在播放时刷新 `/api/fs/get` 返回的签名
2. 普通视频直链：直接填写完整的 http(s) 视频地址，播放时会 302 到原始地址，不依赖 OpenList 配置

OpenList 权限场景：

1. 目录访问密码：在添加或编辑视频时填写“OpenList 目录访问密码”
2. 账号登录资源：配置全局 `ALIST_TOKEN`

新保存的目录密码会加密后存入 D1。旧版本保存的明文密码仍兼容读取，重新保存后会变为密文。

配置了 `ALIST_BASE` 后，只有同一域名下的视频链接会走 OpenList 签名刷新逻辑；其它 http(s) 视频直链会直接播放。批量添加会使用 `ALIST_BASE` 把 OpenList 相对路径拼成完整 URL。
